...
import { PrismaClient, Role, InteractionContext, IncidentType, IncidentSeverity, IncidentStatus, CrisisEventStatus } from "@prisma/client";
...
async function checkAndFlagVolunteer(volunteerId) {
    ...
    if (total >= 3) {
        const fraudCount = reviews.filter(r => FRAUD_KEYWORDS.some(kw => r.text.toLowerCase().includes(kw))).length;
...
async function main() {
    ...
    const crisisDefs = [
        { title: "Mirpur Monsoon Flooding Emergency", type: IncidentType.FLOOD, severity: IncidentSeverity.CRITICAL, loc: "Mirpur Section 10-12, Dhaka", lat: 23.8075, lng: 90.3688, status: CrisisEventStatus.REPORTED, sitRep: "Severe monsoon flooding across Mirpur. Water levels 5+ feet in sections 10-12. Multiple families stranded on rooftops. Boat rescue operations underway. Sewage overflow contaminating floodwater. NDMO coordinating response. 3 shelters opened at local schools.", reportKeys: ["report0", "report1", "report2"], ago: hoursAgo(6) },
        { title: "Khulna Riverside Flood Crisis", type: IncidentType.FLOOD, severity: IncidentSeverity.CRITICAL, loc: "Sonadanga-Daulatpur, Khulna", lat: 22.8350, lng: 89.5500, status: CrisisEventStatus.REPORTED, sitRep: "Rupsha River has breached danger level. 500+ families displaced in Sonadanga and Daulatpur. Army deployed for evacuation. 5 relief distribution centers active. Drinking water contamination confirmed in low-lying areas.", reportKeys: ["report3"], ago: hoursAgo(10) },
        { title: "Sylhet Flash Flood Emergency", type: IncidentType.FLOOD, severity: IncidentSeverity.CRITICAL, loc: "Sylhet-Sunamganj Region", lat: 24.9100, lng: 91.8500, status: CrisisEventStatus.REPORTED, sitRep: "Flash flood from Meghalaya hills has inundated Sylhet-Sunamganj highway and surrounding villages. Army rescue operations active. Air force helicopters requested. 2000+ people stranded. Communication lines disrupted in several areas.", reportKeys: ["report5", "report6"], ago: hoursAgo(8) },
        ...
        { title: "Sadarghat Factory Building Collapse", type: IncidentType.BUILDING_COLLAPSE, severity: IncidentSeverity.CRITICAL, loc: "Sadarghat, Old Dhaka", lat: 23.7080, lng: 90.4050, status: CrisisEventStatus.REPORTED, sitRep: "4-story garment factory partially collapsed. 25 workers rescued alive so far, 4 confirmed dead. NDRF teams using heavy equipment. RAJUK structural team confirms adjacent buildings are safe. Blood donation appeal issued by DMC.", reportKeys: ["report14"], ago: hoursAgo(3) },
        { title: "Khilkhet Flyover Multi-Vehicle Crash", type: IncidentType.ROAD_ACCIDENT, severity: IncidentSeverity.HIGH, loc: "Airport Road, Khilkhet, Dhaka", lat: 23.8310, lng: 90.4225, status: CrisisEventStatus.REPORTED, sitRep: "Bus-CNG collision. 8 injured, 2 critical at DMCH. Traffic diverted via Bashundhara alternate route. Police investigating. Speed and brake failure suspected. Highway police monitoring area.", reportKeys: ["report18"], ago: hoursAgo(2) },
        ...
        { title: "Rampura Dengue Outbreak", type: IncidentType.MEDICAL_EMERGENCY, severity: IncidentSeverity.HIGH, loc: "Rampura, Dhaka", lat: 23.7635, lng: 90.4251, status: CrisisEventStatus.REPORTED, sitRep: "30 confirmed dengue cases in Rampura West. DGHS mosquito eradication drive launched. Fogging scheduled for next 3 days. Blood bank reserves low — O-negative urgently needed. 3 patients in ICU at DMCH.", reportKeys: ["report29"], ago: hoursAgo(20) },
    ];
    const crisisEvents = {};
    ...
    const statusCounts = {
        ACTIVE: crisisDefs.filter(c => c.status === CrisisEventStatus.REPORTED).length,
    ...
    async function createReview(reviewerId, volunteerId, rating, text, context, daysBack, wouldWorkAgain, flagged, flagReasons) {
    ...
    for (let i = 0; i < 4; i++) {
        ...
        await createReview(reviewer.id, volunteers.sohel.id, pick([1, 1, 2]), fraudTexts[i % fraudTexts.length], contexts[i % contexts.length], 4 + i * 5, false, true, ["Contains fraud keywords"]);
    }
    for (let i = 0; i < 2; i++) {
        const reviewer = reviewerPool[(i + 7) % reviewerPool.length];
        await createReview(reviewer.id, volunteers.sohel.id, pick([3, 4]), positiveTexts[(i + 11) % positiveTexts.length], contexts[i % contexts.length], 2 + i * 3, true);
    }
    ...
    for (let i = 0; i < 5; i++) {
        ...
        const isNeg = i < 4;
        await createReview(reviewer.id, volunteers.alamgir.id, isNeg ? pick([1, 2]) : 3, isNeg ? negativeTexts[(i + 2) % negativeTexts.length] : positiveTexts[0], contexts[i % contexts.length], isNeg ? 5 + i * 5 : 40, !isNeg);
    }
    // Munira — fresh volunteer, 0 reviews (empty state)
    // New account flagged review
    await createReview(users.babul.id, volunteers.ayesha.id, 2, "Not impressed with the volunteer service at all. Very slow response.", InteractionContext.OTHER, 0, false, true, ["Account less than 1 day old", "Review text less than 20 characters"]);
