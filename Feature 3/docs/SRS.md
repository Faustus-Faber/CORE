...
| Aspect | Detail |
|:---|:---|
| **Description** | Users can submit a structured review for a volunteer including star rating, detailed text, interaction context, date of interaction, and whether they would work with the volunteer again. The system employs automated fraud detection at both the review level and volunteer level to identify suspicious patterns. Flagged reviews and volunteers are visible to the Admin for moderation. |
| **Actors** | User (as reviewer); Volunteer (as the reviewed subject); Admin (for moderation of flagged reviews and volunteers). |

**Functional Details:**

1. The system shall allow a User to submit a review for a Volunteer, accessible from the volunteer's profile page. The review form shall include:
   ...
   - **Review Text** (text area, required, minimum 20 characters, maximum 2000 characters)
   - **Would You Work Again?** (Yes/No radio buttons, required)
   - **Crisis Event ID** (optional, for linking to a specific crisis event)
2. The system shall prevent:
   - A user from reviewing the same volunteer more than once.
   - A user from reviewing themselves.
   - Volunteers from submitting reviews (only the `User` role can review).
   - Reviews for users who do not have the `VOLUNTEER` role.
3. Upon submission, the system shall store the review linked to the reviewer's User ID and the volunteer's User ID.
4. **Review-Level Fraud Detection:** The system shall automatically flag a review if any of the following conditions are met:
   - The reviewer's account is less than 24 hours old.
   - The review text is shorter than 20 characters.
   - The reviewer has submitted 3 or more reviews within the past 24 hours.
   - The review text contains fraud keywords (scam, fake, fraud, not present, took supplies, stole, liar, dishonest, corrupt, bribe).
5. **Volunteer-Level Fraud Detection:** After each review submission, the system shall re-evaluate the volunteer's profile and flag the volunteer if any of the following conditions are met:
   - The volunteer's average rating is below 2.0 stars across 5 or more reviews.
   - 40% or more of the volunteer's reviews contain fraud keywords.
   - The volunteer has received 3 or more "Would not work again" responses within the past 30 days.
6. Flagged reviews shall be marked with `isFlagged: true` and store the list of flag reasons. The review remains visible but is listed in the Admin Panel for moderation.
7. Flagged volunteers shall have `isFlagged: true` on their profile with a list of flag reasons. A visible **⚠️ Volunteer Under Review** warning badge shall be displayed on their profile page showing the flag reasons.
8. The Admin shall be able to view all flagged reviews and flagged volunteers in the Admin Panel and take the following actions:
   - **For Flagged Reviews:**
     - **Approve:** Remove the flag and flag reasons, making the review a normal, trusted review.
     - **Delete:** Permanently delete the review from the system.
   - **For Flagged Volunteers:**
     - **Clear Flag:** Remove the flag and flag reasons from the volunteer's profile.
     - **Ban Volunteer:** Permanently ban the volunteer from the platform.
9. When a review is approved or deleted, the system shall automatically re-evaluate the volunteer's flag status based on the remaining reviews.
10. Volunteers shall be able to view all reviews submitted for them and see their average rating, but shall **not** be able to delete, edit, or flag reviews submitted by others.
11. The system shall calculate and display the **average rating** for each volunteer based on all their received reviews, shown on the volunteer's profile page.
12. Each review shall display the interaction context, interaction date, and "Would work again" status for transparency.
...
| Aspect | Detail |
|:---|:---|
| **Description** | Authenticated users can search and browse a directory of all registered community volunteers. The directory supports filtering by specialized skills and location, enabling efficient volunteer discovery and coordination. |
| **Actors** | User, Volunteer, Admin. |
...
4. Clicking on a volunteer entry shall navigate to the **volunteer's detailed profile page**, showing:
...
> ⚠️ **Status: Not Implemented** - This feature is planned for future development.
