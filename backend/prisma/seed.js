...
async function main() {
    ...
    for (let i = 0; i < 4; i++) {
        ...
        await createReview(reviewer.id, volunteers.sohel.id, pick([1, 1, 2]), negativeTexts[(i + 4) % negativeTexts.length], contexts[i % contexts.length], 4 + i * 5, false, true, ["Consistently poor performance across multiple deployments"]);
    }
    for (let i = 0; i < 2; i++) {
